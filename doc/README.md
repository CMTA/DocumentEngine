...
["Emission Responsibility"](./ERCSpecification/erc-1643.md) rules — it emits
...
delegating). See
the [Multi-Subject Document Management draft](./ERCSpecification/erc-draft_multi_document_management.md)
...
Generated with Surya — regenerate with the three scripts in [`doc/script`](./script). Diagrams
for **every** file in `src/`, interfaces included, live under [`doc/surya`](./surya); the ones
...
Reports are versioned under [`doc/audits/tools/`](./audits/tools), one directory per release,
...
Three scripts in [`doc/script`](./script) regenerate the diagrams and reports for every `.sol`
